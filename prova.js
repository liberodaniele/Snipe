...
function fnInjectOverviewBar(){
	...
	fnInjectUnits();
	win.$('<tr><td colspan="3">Villaggio bersaglio:<input id="snipe_coord" value="'+defaultCoords+'" class="text-input inactive" size="7" onFocus="this.select()" /></td><td colspan="1">Colpire a:<input id="arrival_time" size="25" class="text-input inactive" value="'+defaultDate+'" onFocus="this.select()" /></td><td><input type="button" value="Vai" onClick="fnCalculateBackTime()" /></td></tr>').insertAfter(win.$('#menu_row2'));
	win.$('<div id="snipe_output"><br/><span>Snipe Calculator</span><br/><span><sub>(snbd and lodi94 modification ' + version + ')</sub><hr></span><br/></div>').insertAfter(win.$('body'));
}
...
function fnCalculateBackTime(){
	...
	srcHTML += "<br/>";
	srcHTML += "<span><sub>(snbd modification " + version + ")</sub><hr></span>";
	srcHTML += "<br/>";
	...
	if(output.length > 0){		
		...
		srcHTML += "<span style=\"color:red;\">Impossibile da raggiungere in tempo</span>";
	}
...
try{
	...
	{
		var author="https://forum.tribals.it/index.php?members/snbd.47338/";
		var win = window;
	...
	{
		UI.InfoMessage('Andando alla panoramica combinata...', 3000, 'successo');
